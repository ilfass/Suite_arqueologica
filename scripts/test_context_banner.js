...
async function testContextBanner() {
  console.log('🔍 Probando banner de contexto y funcionalidad completa...\n');
  ...
  try {
    ...
    // 5. Verificar banner de contexto inicial
    console.log('🔍 Paso 5: Verificando banner de contexto inicial...');
    const initialBanner = await page.evaluate(() => {
      ...
      if (banner) {
        return {
          ...
          hasWarning: banner.textContent.includes('Sin contexto activo')
        };
      }
      return { exists: false };
    });
    console.log('📋 Banner inicial:', initialBanner);
    
    // 6. Verificar contexto inicial
    console.log('🔍 Paso 6: Verificando contexto inicial...');
    ...
    // 7. Verificar herramientas iniciales
    console.log('🛠️ Paso 7: Verificando herramientas iniciales...');
    ...
    // 8. Buscar y hacer clic en el selector de contexto
    console.log('🖱️ Paso 8: Buscando selector de contexto...');
    ...
    if (contextButton) {
      ...
      // 9. Esperar a que aparezca el modal
      console.log('⏳ Paso 9: Esperando modal de contexto...');
      await page.waitForSelector('select', { timeout: 5000 });
      console.log('✅ Modal de contexto apareció');
      
      // 10. Seleccionar un proyecto
      console.log('📋 Paso 10: Seleccionando proyecto...');
      await page.select('select', 'proj-001');
      console.log('✅ Proyecto seleccionado');
      
      // 11. Esperar a que aparezcan las áreas
      console.log('⏳ Paso 11: Esperando áreas...');
      await new Promise(resolve => setTimeout(resolve, 2000));
      
      // 12. Seleccionar un área
      console.log('📋 Paso 12: Seleccionando área...');
      ...
      // 13. Esperar a que aparezcan los sitios
      console.log('⏳ Paso 13: Esperando sitios...');
      await new Promise(resolve => setTimeout(resolve, 2000));
      
      // 14. Seleccionar un sitio
      console.log('📋 Paso 14: Seleccionando sitio...');
      ...
      // 16. Esperar a que se cierre el modal y se actualice
      console.log('⏳ Paso 16: Esperando a que se cierre el modal...');
      await new Promise(resolve => setTimeout(resolve, 3000));
      
      // 17. Verificar banner de contexto después de la selección
      console.log('🔍 Paso 17: Verificando banner de contexto después de la selección...');
      const finalBanner = await page.evaluate(() => {
        const banner = document.querySelector('.bg-green-50, .bg-blue-50');
        if (banner) {
          return {
            exists: true,
            text: banner.textContent.trim(),
            hasContext: banner.textContent.includes('Contexto Arqueológico Activo'),
            isComplete: banner.textContent.includes('Contexto completo')
          };
        }
        return { exists: false };
      });
      console.log('📋 Banner final:', finalBanner);
      
      // 18. Verificar contexto después de la selección
      console.log('🔍 Paso 18: Verificando contexto después de la selección...');
      ...
      // 19. Verificar herramientas después de la selección
      console.log('🛠️ Paso 19: Verificando herramientas después de la selección...');
      ...
      console.log('\n🎯 RESUMEN DE LA PRUEBA DEL BANNER:');
      console.log('=====================================');
      console.log(`Banner inicial: ${initialBanner.exists ? 'Presente' : 'Ausente'}`);
      console.log(`Banner final: ${finalBanner.exists ? 'Presente' : 'Ausente'}`);
      
      if (initialBanner.hasWarning) {
        console.log('✅ Banner inicial muestra advertencia correctamente');
      }
      
      if (finalBanner.hasContext) {
        console.log('✅ Banner final muestra contexto activo');
      }
      
      if (finalBanner.isComplete) {
        console.log('✅ Banner final indica contexto completo');
      }
      ...
      if (finalBanner.exists && finalBanner.hasContext) {
        console.log('✅ SOLUCIÓN: El banner de contexto funciona correctamente');
      } else {
        console.log('❌ PROBLEMA: El banner de contexto no funciona correctamente');
      }
    ...
    console.error('❌ Error durante la prueba del banner:', error);
    await page.screenshot({ 
      path: 'scripts/testing/screenshots/test_context_banner_error.png',
      fullPage: true 
    });
  } finally {
    console.log('\n🏁 Prueba del banner completada. Cerrando navegador en 15 segundos...');
    setTimeout(async () => {
      await browser.close();
    }, 15000);
  }
...
testContextBanner().catch(console.error); 